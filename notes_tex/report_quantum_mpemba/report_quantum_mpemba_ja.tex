...
% ---------------------------------------------------------------
% baselineskip is tightened from the jlreq default: the assignment fixes the
% body size (11pt) and the margins (25mm) but not the leading, and the default
% CJK leading is too generous to fit the argument into the page budget.
\documentclass[a4paper,11pt,onecolumn,baselineskip=15pt]{jlreq}
...
本レポートは，この対立が「時間局所的な生成子が存在するか否か」の一点に帰着し，両機構が矛盾ではなく排他的な適用範囲をもつことを示す．
さらに三状態Markov模型と共鳴準位模型を設定し，動的機構が相互作用にもカオス性にも依存しないことを検証する設計を与え，実効温度の定義依存性という観点から\cite{Wang2024}の限界を批判的に検討して未解決問題を提示する．
\end{abstract}
...
Carolloら\cite{Carollo2021}はこの構造がLindblad方程式にそのまま移植できることを示し，さらに最緩和モードに直交させるユニタリ変換を緩和開始直前に施すことで緩和を指数関数的に加速できるという\emph{制御プロトコル}を提案した．
関連して，可積分系では絡み合い非対称性を秩序変数とする量子ムペンバ効果も議論されている\cite{Ares2023,Rylands2024}．

これらはいずれもMarkov近似---浴の相関時間が零で系--浴結合が弱いこと---に立脚し，扱う模型も可積分あるいは少数自由度の系に限られるため，古典実験で本質的とされる冷却の\emph{速さ}が理論の中で役割を果たさない．
さらに深刻なのは量子カオス系への外挿可能性である．
Hamiltonianがランダム行列として振る舞う系では初期状態の情報が急速にスクランブルされるため\cite{Maldacena2016}，微調整された初期状態の特別さは失われ，$\ovl{2}$に基づく議論の前提そのものが崩れうる．
...
またSYK模型がJT重力と双対であることから\cite{Maldacena2016}，得られた描像は原始ブラックホールの熱化にも示唆を与える．
以下，第\ref{sec:prelim}節で準備を与え，第\ref{sec:model}--\ref{sec:results}節で三つの設定と主張を共通の記法で捉え直す．
第\ref{sec:minimal}節で最小模型による検証を行い，第\ref{sec:discussion}節で仮定の限界を検討して，第\ref{sec:open}節で未解決問題を提示する．
...
無秩序平均により自己エネルギーがGreen関数で閉じ，Schwinger--Dyson方程式
...
\paragraph{非平衡系の実効温度．}
非平衡状態の温度は一意には定まらない．
\cite{Wang2024}は\cite{Eberlein2017}に従い，平衡でのみ厳密なゆらぎ--応答関係（FDT）を低周波数で当てはめて実効温度を定義する．
二時間Green関数の対角スライスをWigner変換
$G(\omega,t)=\int_0^{\infty}\dd{t'}\ee^{\ii\omega t'}G(t+t'/2,\,t-t'/2)$
で周波数表示に移すと，実効逆温度は
\begin{equation}
  \betaeff(t) = \left.
  \frac{2\,\Im\big[\Gkel(\omega,t)\big]}
       {\omega\,\Im\big[\Gret(\omega,t)\big]}
  \right|_{\omega\to0}
  \label{eq:beta_eff}
\end{equation}
で与えられる（$\Gkel$はKeldysh，$\Gret$は遅延Green関数）．
平衡では\eqref{eq:beta_eff}は真の逆温度に一致するが，非平衡でこの量が何を測っているかは自明でない．
この点は第\ref{sec:discussion_temperature}節で検討する．
...
\end{equation}
と分解される．
$\ssv$は$\gen\ssv=0$を満たす定常状態，$\ovl{k}$は初期状態と第$k$モードとの重なりであり，この係数が以下の議論全体の中心となる．

\eqref{eq:spectral_unified}は二つの場合を含む．
古典Markov過程では$(\reig{k},\leig{k})$は遷移速度行列$\Rmat$の右・左固有ベクトルで，対$\ipr{\cdot}{\cdot}$はベクトルの内積である．
Lindblad方程式では$\reig{k},\leig{k}$は$\Lop$と随伴$\Lop^{\dagger}$の固有行列で，対はHilbert--Schmidt内積$\ipr{\leig{k}}{\rho}=\tr[\leig{k}\rho]$となる\cite{Carollo2021}．
すなわち
\begin{align}
...
Wangら\cite{Wang2024}が扱うのは\eqref{eq:generator}の前提が成り立たない状況である．
$N$個のMajorana fermion $\chi_i$からなる系を，$N^2$個のMajorana fermion $\psi_i$からなるSYK浴に結合させた
\begin{equation}
...
\end{align}
で与えられる．
浴の自由度が系の$N$倍のオーダーで多いため浴への反作用は大$N$展開の高次となり，浴は温度$\betabath^{-1}$を保つ理想的な熱浴として振る舞う．
...
\end{equation}
と表される．
第一条件は最緩和モードの一意性を，第二条件は高温の初期状態のほうが遅いモードとの重なりが小さいことを要求する．
同じ論法を加熱に適用すると，$\abs{\ovl{2}^{\,h}}>\abs{\ovl{2}^{\,c}}$のもとで低温の系が速く加熱される逆ムペンバ効果が予言される．
彼らは非対称二重井戸上の一次元拡散過程と三状態系でこれを例示した．
...
を出発点とする．
定常状態からの隔たりをHilbert--Schmidt距離
$\mathcal{E}_t(\rho,\rss)=(\tr[(\ee^{t\Lop}[\rho]-\rss)^2])^{1/2}$
で測ると，長時間漸近形は$\abs{\ovl{2}}\ee^{\Re\gap t}$に比例し，緩和時間は$\tau=1/\abs{\gap}$である．
ここで彼らはLu--Razから一歩踏み込む．
初期状態を受動的に選ぶのではなく，緩和開始直前にユニタリ変換$U$を施して能動的に$\ovl{2}$を消すのである．
すなわち
\begin{equation}
...
Wangら\cite{Wang2024}は\eqref{eq:H_coupled}のクエンチダイナミクスを\eqref{eq:kadanoff_baym}で数値的に解き，実効温度$\betaeff(t)$を\eqref{eq:beta_eff}で追跡した．
...
ところが$\Vsb$が閾値を超えると$\betaeff(t)$は定常値のまわりで振動してMPCが現れ，さらに強い結合では最良のFDT当てはめが負の実効温度を与える領域さえ生じる．
決定的なのは，この閾値を決めるのが初期条件ではなく$\Vsb$のみだという点であり，\eqref{eq:mpemba_condition}が初期温度の関数$\ovl{2}(\betainit)$を通じて条件を与えていたのと対照的である．

同じ模型をLindblad方程式で記述するとMPCは消失する．
Lindblad記述は無限温度の浴に弱く結合したSYK模型に相当するが，厳密計算では無限温度でもGreen関数が$\Jsyk$に比例する有限の幅をもつのに対し，Lindblad近似はこれをDirac delta関数で置き換えてしまう．
すなわち浴の記憶を捨てた瞬間に現象が消える．
また二つの浴の温度差を大きくするとMPCに必要な閾値結合はむしろ増大し，これは可積分系で温度差がムペンバ効果を促進する結果\cite{WangWang2024}と逆で，機構の差異を示す独立な証拠となる．
...
\begin{figure}[htbp]
  ...
  \caption{三状態Markov模型による検証（$E=(0,1,5)$，$\Gamma=(0.3,2.0,0.001)$，$\betabath=0.5$）．
           （a）$\ovl{2}$の初期逆温度依存性．$\betabath$のほかに$\betainit^{*}=0.125$（赤点）でも零となる．
           点線は（b）で用いた二つの初期温度．
           （b）定常状態への$L^1$距離．高温側（橙）は初期に低温側（緑）より遠いが$t=0.31$（灰破線）で追い越す．
           赤破線は$\betainit=\betainit^{*}$の強ムペンバ効果．}
  \label{fig:markov_a2}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{共鳴準位模型による検証（$\edot=1$，$\hyb_0=1$，$\betabath=0.25$）．
           ...
           （b）$\bandw=1$．$\betaeff$は振動し負値もとる．
           帯はFDTフィット窓を$2.0$から$4.0$まで変えたときの値の広がりで，
           二つの初期温度の軌跡の差より一桁大きい．
...
(i)はMarkov近似そのものであり，破れれば\eqref{eq:spectral_unified}が書けない．
(ii)が破れてJordanブロックが生じると緩和は$t^m\ee^{\lambda t}$の形をとり$\ovl{2}$による特徴づけは修正を要するが，Lindbladianは一般に非エルミートであるためこの縮退は例外的とは言えない．
...
$\ovl{2}=0$としても短時間領域では高次モードが寄与するため，緩和が実際に速いかどうかは自明ではない．
\cite{Carollo2021}の主張が意味をもつのは観測時間が$1/\abs{\Re\lambda_3}$より十分長い場合に限られ，逆に言えばこのプロトコルは古典ムペンバ実験が本質的に短時間の現象であることと噛み合っていない．
さらに\eqref{eq:optimal_unitary}は$\leig{2}$の完全な知識を前提とするが，これは$\gen$の全スペクトルを解くことと同等であり，設計コストが加速による利得を上回る可能性は検討されていない．

\subsection{実効温度の定義依存性}
\label{sec:discussion_temperature}

\cite{Wang2024}に対する最も重い批判は，MPCが\emph{観測量の定義に依存する見かけの現象}である可能性を排除できていない点にある．
\eqref{eq:beta_eff}の$\betaeff(t)$は平衡でのみ成立するFDTを非平衡に外挿して得られる量である．
実際\cite{Wang2024}自身が，系の分布関数$\Feff(\omega,t)$と浴のFermi--Dirac分布$f_{\mathrm{bath}}(\omega)$との距離
\begin{equation}
...
共鳴準位模型では$\Dbath(t)$が単調に減少する一方で$\betaeff(t)$は振動し，しかも軌跡の隔たりはフィット窓による系統的不定性より一桁小さかった．
少なくともこの模型では，交差は状態の性質ではなく温度計の性質である．
実効温度の抽出に伴う系統誤差を評価して軌跡の隔たりと比較することが，MPCの実在性を主張するために必要な最小限の手続きであろう．
なお元来のムペンバ効果もまた温度計の読みで定義される以上，この問いはMPCという概念そのものに向けられるべきものであり，スペクトル機構がこれを免れているのは$\ovl{2}$が温度ではなく状態に対して定義されているからである．

\subsection{普遍性主張の検証状況}
\label{sec:discussion_universality}

\cite{Wang2024}は結論部で，MPCの機構をスクランブル時間$\tau_S$とエネルギー緩和時間$\tau_E$の分離$\tau_S\ll\tau_E$に帰し，これがカオス束縛に近い系一般に成り立つと主張する．
しかし定量的証拠は与えられていない．
結合系におけるOTOCもLyapunov指数$\lambda_L$も計算されておらず，$\tau_S$と$\tau_E$のいずれも数値的に評価されていない．
さらに$\Vsb$を増やせば$\lambda_L$は減少することが知られているから，MPCが現れる強結合領域はむしろカオス束縛から遠ざかる領域である可能性がある．

第\ref{sec:minimal_rlm}節の結果はこの主張に否定的である．
共鳴準位模型は二次形式でありスクランブルもLyapunov指数も存在しないが，$\betaeff(t)$の振動と負値は\cite{Wang2024}と同様に現れた．
したがって$\tau_S\ll\tau_E$は現象の\emph{必要条件ではない}．
...
第\ref{sec:discussion_temperature}節の議論から次の未解決問題を提示する．
...
この問題が切実であるのは，第\ref{sec:minimal_rlm}節で$\Dbath(t)$が単調に減少する一方で$\betaeff(t)$が振動し，軌跡の隔たりが温度計の不定性に埋もれることを確かめたからである．
定義非依存な特徴づけが存在しないのであればMPCは物理現象ではなく温度計の性質ということになり，動的機構という主張自体が意味を失う．
逆に存在するのであればMarkov極限で$\ovl{2}$に帰着すべきであり，二つの機構を真に統一する量となる．
候補としては記憶核の汎関数や$\Feff(\omega,t)$の非単調性を捉える高次モーメントが考えられるが，$\ovl{2}$との接続は明らかでない．

\paragraph{まとめ．}
量子ムペンバ効果をめぐる二つの対立する描像を，生成子のスペクトル分解という共通言語のもとで再構成し，両者が$\gen$の存否によって排他的に棲み分けることを示した．
最小模型による検証は，スペクトル機構が$\ovl{2}$の符号反転として確認できる一方，動的機構についてはカオス性が不要であることを示すと同時に，交差そのものが実効温度の定義の不定性に埋もれることを明らかにした．
動的機構を物理現象として確立するには，まずこの不定性の評価が必要である．
...
% TODO: /code 以下のスクリプトが完成した時点で挿入する．
% 課題の指示により，コードの掲載または公開リポジトリへのリンクが必要．
