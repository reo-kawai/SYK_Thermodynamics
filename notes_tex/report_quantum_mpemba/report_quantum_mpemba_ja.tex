...
\begin{abstract}
Mpemba効果の起源をめぐる二つの整合しない描像を，共通の記法のもとで一貫した議論として再構成する．
Lu--Raz\cite{Lu2017}による古典Markov過程の定式化と
Carolloら\cite{Carollo2021}によるLindblad方程式への拡張は，
緩和生成子のスペクトル分解における最緩和モードとの重なり係数$\ovl{2}$で現象を特徴づける点で
...
高温の水が低温の水よりも速く凍るというMpemba効果\cite{Mpemba1969}は，準静的熱力学では説明できない緩和現象の代表例である．
...
彼らはMarkov的緩和において，確率分布を遷移速度行列の固有モードに分解したときの第二モードの係数$\ovl{2}$が緩和の遅さを支配することを見出し，Mpemba効果の十分条件を導いた．
...
また二浴の温度差を大きくすると閾値結合はむしろ増大し，可積分系で温度差がMpemba効果を促進する結果\cite{WangWang2024}と逆で，機構の差異を示す独立な証拠となる．
...
\begin{figure}[htbp]
  ...
  \caption{三状態Markov模型による検証（$E=(0,1,5)$，$\Gamma=(0.3,2.0,0.001)$，$\betabath=0.5$）．
           ...
           （b）定常状態への$L^1$距離．高温側（橙）は初期に低温側（緑）より遠いが$t=0.31$（灰破線）で追い越す．赤破線は$\betainit=\betainit^{*}$の強Mpemba効果．}
  \label{fig:markov_a2}
\end{figure}
...
少なくともこの模型では交差は状態ではなく温度計の性質であり，系統誤差の評価と軌跡の隔たりとの比較がMPCの実在性を主張する最小限の手続きであろう（元来のMpemba効果も温度計の読みで定義される）．
...
\paragraph{まとめ．}
量子Mpemba効果をめぐる二つの対立する描像を生成子のスペクトル分解という共通言語のもとで再構成し，両者が$\gen$の存否によって排他的に棲み分けることを示した．
...
本レポートの作成にあたり生成AI（Claude）を，文献の探索，本文執筆の補助と日本語表現の校正，
...
なお\cite{Wang2024}の本文では\eqref{eq:fdt_general}に相当する式が逆符号で書かれているが，同論文の$\betaeff$の定義式は\eqref{eq:beta_eff}と同じ正符号であり，両者は整合しない．
本レポートでは，弱結合の共鳴準位模型において$\betaeff(t\to\infty)$が$\betabath$に相対誤差$5\times10^{-5}$で一致することを数値的に確認して\eqref{eq:fdt_general}の符号を確定した（第\ref{sec:minimal_rlm}節）．
